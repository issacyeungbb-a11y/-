...
## 4) 如果 PR 顯示 `index.html` 衝突（如你附圖）
這代表你的分支和目標分支都改了同一段 `index.html`。

### 手機版（GitHub 網頁）
1. 在 PR 頁按 **Resolve conflicts**。
2. 進入 `index.html` 後，刪除所有衝突標記：
   - `<<<<<<<`
   - `=======`
   - `>>>>>>>`
3. 保留最新版本內容（建議保留你這次訓練系統版本）。
4. 按 **Mark as resolved** → **Commit merge**。
5. 回到 PR，確認 **This branch has no conflicts** 才可 Merge。

### 指令版（電腦）
```bash
git fetch origin
git merge origin/main
# 打開 index.html，手動解衝突後
# 確認已沒有 <<<<<<< ======= >>>>>>>
git add index.html
git commit -m "Resolve index.html merge conflict"
git push
```

> 小貼士：`index.html` 是單檔大檔案，容易衝突；每次更新前先同步主分支可減少衝突。
